...
| `AI_IMAGE_PROVIDER` | 否 | 生图提供方。设置为 `cloudbase` 时直连 CloudBase 生图云函数；留空时走 OpenAI 兼容生图接口。 |
| `CLOUDBASE_ENV_ID` | CloudBase 生图时必填 | CloudBase 环境 ID。 |
| `CLOUDBASE_REGION` | 否 | CloudBase 地域，例如 `ap-shanghai`。 |
| `CLOUDBASE_ACCESS_KEY` | CloudBase 生图时必填 | CloudBase Web/服务端访问密钥。也可改用 `CLOUDBASE_SECRET_ID` 与 `CLOUDBASE_SECRET_KEY`。 |
| `CLOUDBASE_IMAGE_FUNCTION_NAME` | CloudBase 生图时必填 | CloudBase 控制台一键创建的生图云函数名称。 |
| `AI_GATEWAY_IMAGE_API_KEY` | 否 | OpenAI 兼容生图接口 API Key；留空则复用 `AI_GATEWAY_API_KEY`。 |
| `AI_GATEWAY_IMAGE_BASE_URL` | 否 | OpenAI 兼容生图接口 Base URL；留空则复用 `AI_GATEWAY_BASE_URL`。 |
| `AI_GATEWAY_IMAGE_MODEL` | 否 | OpenAI 兼容生图模型 ID。 |

本模板遵循 OpenAI 兼容标准 —— 可指向 Makers Models 或任何兼容提供商。

### CloudBase 生图直连

如果生图额度来自微信云开发 CloudBase，请在 CloudBase 生图模型页面点击「一键创建云函数」，然后配置：

```bash
AI_IMAGE_PROVIDER=cloudbase
CLOUDBASE_ENV_ID=你的环境ID
CLOUDBASE_REGION=ap-shanghai
CLOUDBASE_ACCESS_KEY=你的CloudBase AccessKey
CLOUDBASE_IMAGE_FUNCTION_NAME=你的生图云函数名
```

启用后，提示词深度润色仍使用 `AI_GATEWAY_MODEL` 文本模型；真正生成图片会直连 CloudBase 云函数，不再经过 9Router/OpenAI 兼容生图接口。
