...
export function getDailyRequestLimit(env = {}) {
  const envSource = env?.DEFAULT_DAILY_REQUEST_LIMIT ? env : (typeof process === "object" && process.env ? process.env : {});
  const parsed = Number(envSource.DEFAULT_DAILY_REQUEST_LIMIT);
...
function parseUsageRecord(raw, date, limit) {
  ...
  try {
    ...
    return {
      ...
      limit: limit, // Always use the latest limit from env instead of cached KV
